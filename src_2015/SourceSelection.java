...
public class SourceSelection {
	...
	public void doSourceSelection(List<StatementPattern> stmts) {
		...
		HashMap<StatementPattern, HashSet<TreeSet<Endpoint>>> candidateSources= null;
		   ...
		   if(sourceSelectionStrategy.endsWith("Replicated")){
		       	
			fss.performSourcePerTriple();
			candidateSources=fss.getFedraCandidateSources();
		   }else{
		...
		for (StatementPattern stmt : stmts) {
			...
			SubQuery q = new SubQuery(stmt);
		        if (sourceSelectionStrategy.startsWith("Fedra")) {
                            if (selectedSources.containsKey(stmt)) { 
		...
		for (StatementPattern stmt : stmtToSources.keySet()) {
			...
			if (sources.size()>1) {
			   ...
			   if(sourceSelectionStrategy.endsWith("Replicated")){	
			//	System.out.println("stmt info : "+queryInfo);
				FedraStatementSourcePattern fstmtNode = new FedraStatementSourcePattern(stmt, queryInfo,candidateSources);
			//StatementSourcePattern stmtNode = new StatementSourcePattern(stmt, queryInfo);	
			  	if(stmt.getPredicateVar().getName().contains("-1")){   
					...
					//we get the first of all subset to be sure to have all bindings for the first tripple.
					for(int i = 0 ; i <ssTmpL.size(); i++){
						fstmtNode.addStatementSource(ssTmpL.get(i).get(0));
					}

				}else{
					 for (StatementSource s : sources){
                                        	fstmtNode.addStatementSource(s);
                                	}

				}
				
					stmt.replaceWith(fstmtNode);
