...
# path to the differente output to process
setupResultsPath = {
						"fedra" : "examples/outputFedra",
						"fedra-pbj-pre" : "examples/outputFedraPBJ",
						"fedra-pbj-post" : "examples/outputFedraPBJ",
						"fedra-pbj-hybrid" : "examples/outputFedraPBJ"
					}

for name, filepath in setupResultsPath.items():
	# extract the datas generated by the setup
	with open(filepath, "r") as outputSetup:
		# open the csv writer and write the header
		with open("output_csv/" + name + ".csv", "w", newline="") as csvfile:
