...
with open(config["outputs_csv"]["fedra"], "r", newline="") as csvfile:
...
del config["outputs_csv"]["fedra"]
for name, filepath in config["outputs_csv"].items():
	# retrieve the output from the current file
	with open(config["outputs_csv"][name], "r", newline="") as csvfile:
		csvreader = csv.reader(csvfile, delimiter=";", quotechar="|")
	...
	with open(config["outputs_delta"][name], "w", newline="") as csvfile:
